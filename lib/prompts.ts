...
export function buildUserPrompt(question: string, contextText: string) {
  const formattingInstruction = question.toLowerCase().includes("return only the titles")
    ? "Output only the requested distinct article titles, one per line, with no explanation."
